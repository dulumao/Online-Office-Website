...
window.onload = openNavList();
    ...
    <div class="content">
        ...
        <div class="header">
            <h1 class="page-title">更改区县管理账户</h1>
        </div>
        
        <ul class="breadcrumb">
            <li>区县管理部门<span class="divider">/</span></li>
            <li class="active">更改区县管理账户</li>
        </ul>
...
<div class="row-fluid">
    <div class="block">
        <a href="#page-stats" class="block-heading" data-toggle="collapse">更改区县管理账户</a>
        <div id="page-stats" class="block-body collapse in">
        <form action="index.php?m=Admin&a=admin_list" method="post">	
        	<div class="well">
            	<div>
					<input name="id" value="<?php echo ($li["id"]); ?>" type="hidden"/>
                	<label>用户名:</label>
                    <input type="text" value="<?php echo ($li["user_name"]); ?>" name="user_name"class="input-xlarge">
                    <span>推荐使用“区县名+单位名称”的格式</span>
                	<label>密码:</label>
                    <input type="text" value="<?php echo ($li["password"]); ?>" name="password" class="input-xlarge">
                    <label>负责区域：</label>
                    <select class="input-xlarge" name="region">
                      <?php if(is_array($c)): $i = 0; $__LIST__ = $c;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><option value="<?php echo ($c["region_name"]); ?>"><?php echo ($c["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                     
                	</select>                    
                </div>
            </div>
