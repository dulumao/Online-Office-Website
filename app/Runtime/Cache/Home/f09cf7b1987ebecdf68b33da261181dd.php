...
<div class="row-fluid">
    <div class="block">
        <a href="#page-stats" class="block-heading" data-toggle="collapse">项目修改（第二步：上传附件。）</a>
        <div id="page-stats" class="block-body collapse in">
        	<div class="well">
                ...
                <table class="table">
				  ...
				  <?php if(is_array($li)): $i = 0; $__LIST__ = $li;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><a href="index.php?m=Unit&a=getfile&id=<?php echo ($li["id"]); ?>&fid=<?php echo ($li["fid"]); ?>"><?php echo ($li["name"]); ?></a></td>                     
                      <td><?php echo ($li["time"]); ?></td>
                      <td>
                          <a href="index.php?m=Unit&a=removefile&id=<?php echo ($li["id"]); ?>&fid=<?php echo ($li["fid"]); ?>"><i class="icon-remove"></i></a>
                      </td>
        	...
        	<div class="well">
            	<h3>上传新文件</h3>
          	 	<form action="index.php?m=Unit&a=oldproject_submit&id=<?php echo ($id); ?>" method="post" enctype="multipart/form-data">
